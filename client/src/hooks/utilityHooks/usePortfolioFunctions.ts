import {usePortfolio, IPortfolioCurrency} from "contexts";
...
export const usePortfolioFunctions = () => {
    ...
    const addPortfolioCurrency = (currency: ICurrency, amount: number): void => {

        if (portfolio.currencies.some((saved_currency: IPortfolioCurrency) => saved_currency.id === currency.id)) {
            portfolio.currencies.map((saved_currency: IPortfolioCurrency) => {
    ...
    const removePortfolioCurrency = (currency_id: string): void => {
        ...
        if (portfolio.currencies.some((currency: IPortfolioCurrency) => currency.id === currency_id)) {
            portfolio.currencies = portfolio.currencies.filter((saved_currency: IPortfolioCurrency) => {
    ...
    const updatePortfolioCurrencies = (currencies: Array<any>): void => {
        ...
        portfolio.currencies = portfolio.currencies.map((saved_currency: IPortfolioCurrency) => {
            ...
            const currency = currencies.find((curr: ICurrency) => curr.id === saved_currency.id);
